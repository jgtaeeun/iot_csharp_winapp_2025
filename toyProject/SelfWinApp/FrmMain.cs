...
namespace SelfWinApp
{
    public partial class FrmMain : Form
    {
        ...
        List<Question> QuizList;    //기출문제
        int questionCount;          //내가 풀 문제 개수
        List<int> QNum;             //내가 풀 문제 개수별 랜덤문제 배정
        List<int> answeredQuestions = new List<int>();   //내가 푼 문제, 푼 문제를 다시 풀면 카운팅하지 않도록
        int currentQuestionIndex = 0;  // 현재 보여지는 문제 번호
        int progressNum = 0; //지금까지 푼 문제 개수
        ...
        {   //문제할당
            ...
            if (questionCount > 3)
            {
                ...
                for (int i = 0; i < questionCount - 3; i++)
                {
                    num = rand.Next(0, 3);
                    QNum.Add(num);
                }
        ...
        {
            int questionIndex = QNum[index];      //기출문제 1~3 중 무엇인지 확인
            var question = QuizList[questionIndex];//기출문제 저장된 곳에서 기출문제(답, 정답, 해설) 가져옴
            int correctAnswer = question.CorrectAnswer; //문제의 정답

            // 틀린문제리스트에 기출문제번호추가
            wrongNum.Add(questionIndex);

            progressNum++;
            TxtAnswer.Text = question.Answer;   //기출문제 해설
            progressBar1.Value = (int)((progressNum / (double)numericUpDown1.Value) * 100);   //진행률
            //LblRate.Text = $"{correctNum}/{numericUpDown1.Value}";          //정답률
            answeredQuestions.Add(index);   //푼 문제리스트에 저장
        }
        ...
        private void BtnShowAnswer_Click(object sender, EventArgs e)
        {
            ...
            LblRate.Text = $"{correctNum}/{numericUpDown1.Value}";
            answeredQuestions.Add(currentQuestionIndex); //이미 푼 문제는 진행률에 포함시키지 않기 위해서
        }

        //틀린문제 보기-이미지로 문제만 모달리스창을 통해 볼 수 있음.
        private void BtnShowWrong_Click(object sender, EventArgs e)
        {
        ...
        private void BtnSave_Click(object sender, EventArgs e)
        {
            ...
            if (saveFileDialog1.ShowDialog() == DialogResult.OK)
            {
                ...
                richTextBox1.AppendText("\r\n" + selectedDate.ToString("yyyy-MM-dd HH:mm:ss"));
        ...
        {
            BtnSave_Click(sender, e);  // 동일한 sender와 e를 전달
        }
